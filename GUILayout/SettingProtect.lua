...
function SettingProtect.main()
    ...
    for idx,data in ipairs(GUIShare.SetCfg.SetProtectCfg) do
        if data then
            ...
            -- 输入框
            local input = SettingProtect.onCreateInput(CheckBox)
            GUI:TextInput_setString(input, cdtime)
            GUI:Win_SetParam(input, {_ID = id, _minCD = minCD, _maxCD = maxCD})
            GUI:TextInput_addOnEvent(input, SettingProtect.onInputEvent)
...
-- 输入框事件
function SettingProtect.onInputEvent(sender, eventType)
