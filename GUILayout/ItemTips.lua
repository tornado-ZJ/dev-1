...
local function IsOpenSwitch(switchType, StdMode)
    ...
    if not ItemTips._Switch then
        ItemTips._Switch = {}
        local BackpackGuide = SL:GetGamaDataCfg().BackpackGuide
...
function ItemTips.GetEquipTips(data)
    ...
    if checkSet and (from == ItemFrom.BAG or from == ItemFrom.AUTO_TRADE or _IsHeroEquip) then
...
-- 下拉箭头
function ItemTips.CreateButtonArrow(parent)
    local ImgArrow = GUI:Image_Create(parent, "ImgArrow", GUI:getContentSize(parent).width/2, 0, "res/public/btn_szjm_01_1.png")
    GUI:setAnchorPoint(ImgArrow, 0.5, 0.5)
    GUI:setScale(ImgArrow, 0.6)
    GUI:setRotation(ImgArrow, 180)
    GUI:runAction(ImgArrow, GUI:ActionRepeatForever(GUI:ActionSequence(GUI:ActionFadeTo(0.2, 125), GUI:ActionFadeTo(0.2, 255), GUI:DelayTime(0.3))))
end
...
function ItemTips.CreateEquipPanel(data, itemData, isWear, isCompare)
    ...
    local r_name = GUI:RichText_Create(ListView, "r_name", 0, 0, title, richWidth, _TextSize, SL:GetColorByID(color))
    ...
    if isBindUser then
        local r_bindUser = GUI:RichText_Create(ListView, "r_bindUser", 0, 0, string.format("已绑定[%s]", itemData.BindInfo), richWidth, nil, color)
        ItemTips.CreateIntervalPanel(ListView, _DefaultSpace, true)
    ...
    if isBind then
        local r_bind = GUI:RichText_Create(ListView, "r_bind", 0, 0, "已绑定", richWidth, nil, color)
        ItemTips.CreateIntervalPanel(ListView, _DefaultSpace, true)
    ...
    local Icon = GUI:ItemShow_Create(IconBg, "Icon", IconBgSz.width / 2, IconBgSz.height / 2, {
        itemData = itemData, index = itemData.Index
    })
    ...
    if typeStr and string.len(typeStr) > 0 then
        ...
        local r_type = GUI:RichText_Create(Icon, "r_type", wx, wy, typeStr, richWidth, nil, 1038)
    ...
    if needStr then
        ...
        local r_need = GUI:RichText_Create(Icon, "r_need", wx, wy, needStr, richWidth, nil, 1038)
    ...
    if jobStr then
        ...
        local wy = IconBgSz.height * 0.7
        local r_job = GUI:RichText_Create(Icon, "r_job", wx, wy, jobStr, richWidth, nil, 1038)
    ...
    --     local wx = IconBgSz.width + 10
    --     local wy = IconBgSz.height * 0.7
    --     local r_weight = GUI:RichText_Create(Icon, "r_weight", wx, wy, str, richWidth, nil, 1038)
    ...
    --     local wx = IconBgSz.width + 10
    --     local wy = IconBgSz.height * 0.3
    --     local r_mode = GUI:RichText_Create(Icon, "r_mode", wx, wy, aStr, richWidth, nil, 1038)
    ...
    if string.len(baseAttStr) > 0 then
        local r_att1 = GUI:RichText_Create(ListView, "r_att1", 0, 0, baseAttStr, richWidth, nil, 1038)
        ItemTips.CreateIntervalPanel(ListView, _DefaultSpace, true)
    ...
    if not GUIShare.IsAddToBaseAtt and attExtra and next(attExtra) then
        ...
        local r_extra = GUI:RichText_Create(ListView, "r_extra", 0, 0, extraAttStr, richWidth, nil, 1038)
        ItemTips.CreateIntervalPanel(ListView, _DefaultSpace, true)
    ...
    for k,suitID in ipairs(suitArr) do
        ...
        if suitID > 0 then
            ...
            if ListSuit then
                local r_suit = GUI:RichText_Create(ListSuit, "r_suit"..k, 0, 0, suitStr, richWidth, nil, 1038)
                ...
                local r_suit = GUI:RichText_Create(ListView, "r_suit"..k, 0, 0, suitStr, richWidth, nil, 1038)
                ItemTips.CreateIntervalPanel(ListView, _DefaultSpace, true)
    ...
    if desc and string.len(desc) > 0 then
        local r_desc = GUI:RichText_Create(ListView, "r_desc", 0, 0, desc, ww + 10, _TextSize, SL:GetColorByID(1038))
    ...
    if isScroll then
        GUI:ListView_addOnScrollEvent(ListView, function (sender)
            if sender then
                -- 到底隐藏箭头
                local isBottom = GUI:ListView_getInnerContainer(sender):getPositionY() == 0
                if isBottom then
                    GUI:setVisible(GUI:getChildByName(ListBg, "ImgArrow"), false)
                else
                    GUI:setVisible(GUI:getChildByName(ListBg, "ImgArrow"), true)
                end
            end
        end)
        GUI:setTouchEnabled(ListView, true)
        ItemTips.CreateButtonArrow(ListBg)
    end
...
function ItemTips.GetNeedStr(itemData)
    local canUse, strList = _IsHeroEquip and SL:CheckItemUseNeed_Hero(itemData) or SL:CheckItemUseNeed(itemData)
    ...
    if (strList and next(strList)) then
        ...
        return string.format("<font color='%s'>%s</font>%s", getColor(v.can), "需求：", str)
    end
...
function ItemTips.GetSuitStr(suitID, itemData)
        ...
        if eData and eData.Suit and string.len(eData.Suit) > 0 then 
            ...
            for i,v in ipairs(suitArr) do
                if v and string.len(v) > 0 then
                    ...
                    if posSuitConfig and posSuitConfig.suittype == suitCfgData.suittype and posSuitConfig.level >= suitCfgData.level then
                        isWear = true
                        eName = eData.Name
                        break
                    end
    ...
    local function ParseColor(txtStr, colorIdx)
        ...
        return tonumber(colorArry[colorIdx]) and colorArry[colorIdx] or colorArry[1], showStrt
    end
    ...
    for i,pos in ipairs(equipPos) do
        pos = tonumber(pos)
        if pos then
            local isWear, equipName = CheckIsWearEquipPos(suitCfgData, pos)
            if isWear then
    ...
    if titleStr and string.len(titleStr) > 0 then
        ...
        local color, str = ParseColor(titleStr, wearCount >= suitCount and 2 or 1)
        suitStr = string.format("<font colorStyleID='%s'>%s (%s/%s)</font><br>", color, str, mCount, suitCount)
    end
    ...
    for _,nameStr in ipairs(nameStrs) do
        if nameStr and string.len(nameStr) > 0 then
            ...
            local color, str = ParseColor(nameStr, isWear and 2 or 1)
            suitStr = suitStr .. string.format("<font colorStyleID='%s'>%s</font><br>", color, str)
        end
    end
    ...
    if descStr and string.len(descStr) > 0 then
        local color, str = ParseColor(descStr, wearCount >= suitCount and 2 or 1)
        suitStr = suitStr .. string.format("<font colorStyleID='%s'>%s</font><br>", color, str)
    end
...
function ItemTips.CreateItemPanel(data, itemData)
    ...
    local r_name = GUI:RichText_Create(ListView, "r_name", 0, 0, name, richWidth, _TextSize, SL:GetColorByID(color))
    ItemTips.CreateIntervalPanel(ListView, _DefaultSpace, true)
    ...
    if hmpStr and string.len(hmpStr) > 0 then
        local r_hmp = GUI:RichText_Create(ListView, "r_hmp", 0, 0, hmpStr, richWidth, nil, color)
        ItemTips.CreateIntervalPanel(ListView, _DefaultSpace, true)
    ...
    local aStr = ItemTips.GetOtherAttrStr(itemData)
    if aStr then
        local r_mode = GUI:RichText_Create(ListView, "r_mode", wx, wy, aStr, richWidth, nil, 1038)
        GUI:setAnchorPoint(r_mode, 0, 0.5)
    ...
    local btnhh = 0
    local isBtn = false
    if data and data.from == ItemFrom.BAG then
        -- 使用
    ...
    local isScroll = visiblehh > _TipsMaxH and true or false
    if isScroll then
        GUI:ListView_addOnScrollEvent(ListView, function (sender)
            if sender then
                -- 到底隐藏箭头
        ...
        [2] = {
            ...
            func = function (data)
                if not SL:IsBagFull(true) then
                    global.Facade:sendNotification(global.NoticeTable.Layer_Split_Open, {itemData = SL:GetItemDataByMakeIndex(data.MakeIndex)})
                end
                SL:CloseItemTips()
            end
        },
