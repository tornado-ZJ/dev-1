...
function SettingBattle.main()
    ...
    for key,datas in ipairs(GUIShare.SetCfg.SetBattleCfg) do
        if showAll or roleJob == (key-2) then
            local k = 0
...
function SettingBattle.createSkillPop(data)
    ...
    if not PopView then
        PopView = GUI:Layout_Create(parent, "PopView", width / 2, height / 2, width, height)
...
function SettingBattle.RefreshItem(data)
    ...
    local item = GUI:getChildByName(SettingBattle._FrameList, "CheckBox"..data.id)
    if not item then
