...
local cfg = SL:Require("csvcfg/cfg_shouchong")
local data = SL.FormParam[1]

local last = nil
shouchong.main = function()
    last = nil
    local _parent = GUI:Win_Create("QSQ_shouchong", 0, 0, 0, 0, false, false, true, false) --节点
    if _parent then
        ...
        if bg_close then
            GUI:setContentSize(bg_close, {width = w, height = h})   -- 黑色背景大小 设置为屏幕大小
            GUI:setTouchEnabled(bg_close, true)         -- 设置黑色背景是否可点击
            GUI:addOnClickEvent(bg_close, function()    --设置点击响应 函数
    ...
    shouchong.test(data)
    SL:SubmitForm("B/首充福利表单", SyncResponse)
    end
...
--                 SL:SubmitForm("B/首充豪礼表单", "LingQu",num)
--             end
--         end
-- end
...
-- end
    ...
    -- --- 领取按钮
    -- if num == 1 then
    --     local btn_close8 = GUI:Button_Create(nil, "btn_close8", 700, 160,"res/custom/shouchong/btn_shouchong_05.png")
    --     if btn_close8 then
    --         GUI:setAnchorPoint(btn_close8, 0.5, 0.5) ---设置中心点
    --         GUI:Button_loadTexturePressed(btn_close8, "res/custom/shouchong/btn_shouchong_05_1.png")
    --         GUI:addOnClickEvent(btn_close8, function()
    --             SL:SubmitForm("B/首充豪礼表单", "LingQu",num)
    --             SL:ShowSystemTips("领取成功")
    --         end)
    --     end

    -- --- 已领取按钮
    -- if num == 2 then
    --     local btn_close9 = GUI:Button_Create(nil, "btn_close9", 700, 160,"res/custom/shouchong/backinvest_btn_66.png")
    --     if btn_close9 then
    --         GUI:setAnchorPoint(btn_close9, 0.5, 0.5) ---设置中心点
    --         GUI:Button_loadTexturePressed(btn_close9, "res/custom/shouchong/backinvest_btn_66.png")
    --         GUI:Text_Create(button, "title", 40, 9, 25, "#D6C6AD", "已领取")
    --         GUI:addOnClickEvent(btn_close9, function()
    --         GUI:Button_setBright(btn_close9,false)
    --         GUI:setTouchEnabled(btn_close9, false)
    --             SL:ShowSystemTips("亲您已经领过了")
    --         end)
    --     end

    --- listview
    local listview = GUI:GetWindow(nil, "QSQ_shouchong/listview")
    GUI:removeAllChildren(listview)     --清空，移除其下所有子控件
    if listview then
        
    --- 清空
    -- GUI:ListView_removeAllItems(listview)
    -- GUI:removeAllChildren(listview)      -- 移除其下所有子控件
