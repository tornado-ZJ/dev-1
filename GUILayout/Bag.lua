...
function Bag.main( page )
    ...
    SL:RegisterLUAEvent(LUA_EVENT_REF_ITEM_LIST, "BagReset", Bag.RefreshItemList)
    SL:RegisterLUAEvent(LUA_EVENT_ITEM_POS_CHANGE, "BagPosChange", Bag.ItemPosChange)
    SL:RegisterLUAEvent(LUA_EVENT_PLAYER_EQUIP_CHANGE, "PlayerEquipChange", Bag.RefItemBeStrongMask)
    SL:RegisterLUAEvent(LUA_EVENT_BAG_ITEM_CAHNGE, "BagItemChange", Bag.BagItemChange)

    SL:RegisterLUAEvent(LUA_EVENT_ITEM_MOVING, "Moving", Bag.OnItemMoving)
    SL:RegisterLUAEvent(LUA_EVENT_ITEM_MOVE_END, "MoveEnd", Bag.OnItemMoveEnd)
end

function Bag.InitPage(parent)
    SL:release_print("背包最大格子数量：", Bag._openNum)
...
function Bag.LoadUnOpenGrid(lockNum)
    ...
    for i=clockPos, Bag._PerPageNum do
        ...
        local lock = GUI:Clone(Bag._UI_ImageLock)
        GUI:setPosition(lock, posX, posY)
        GUI:setVisible(lock, true)
        GUI:addChild(Bag._UI_ScrollView, lock)
...
function Bag.CreateItem(data, pos)
    local YPos = math.floor((pos-1) / Bag._Col)
