...
function TradePlayerBaseAtt.main()
    local parent  = GUI:Attach_Parent()
    local attachW = 348
    local attachH = 478
    TradePlayerBaseAtt.attachW = attachW

    local _ResPath = "res/private/player_main_layer_ui/player_main_layer_ui_mobile/"
    TradePlayerBaseAtt.ResPath = _ResPath
    ...
    TradePlayerBaseAtt.ListView = ListView

    TradePlayerBaseAtt.RefreshAttr(true)
end

function TradePlayerBaseAtt.RefreshAttr(isJump)
    local List = TradePlayerBaseAtt.ListView

    -- 职业
    local jobName = SL:GetMetaValue("T.M.JOB")
    local item_1 = GUI:getChildByName(List, "item_1")
    if not item_1 then
        item_1 = GUI:Layout_Create(List, "item_1", 0, 0, TradePlayerBaseAtt.attachW  - 10, 30, true)
        local num_bg = GUI:Image_Create(item_1, "num_bg", 175, 15, TradePlayerBaseAtt.ResPath.."1900015004.png")
    ...
    -- 等级
    local level = SL:GetMetaValue("T.M.LEVEL")
    local item_2 = GUI:getChildByName(List, "item_2")
    if not item_2 then
        item_2 = GUI:Layout_Create(List, "item_2", 0, 0, TradePlayerBaseAtt.attachW  - 10, 30, true)
        local num_bg = GUI:Image_Create(item_2, "num_bg", 175, 15, TradePlayerBaseAtt.ResPath.."1900015004.png")
        GUI:setContentSize(num_bg, {width = 110, height = 26})
        GUI:Image_setScale9Slice(num_bg, 30, 30, 6, 6)
        GUI:setAnchorPoint(num_bg, 0, 0.5)
    ...
    -- 当前经验
    local curExp = SL:GetMetaValue("T.M.EXP")
    local item_3 = GUI:getChildByName(List, "item_3")
    if not item_3 then
        item_3 = GUI:Layout_Create(List, "item_3", 0, 0, TradePlayerBaseAtt.attachW  - 10, 30, true)
        local num_bg = GUI:Image_Create(item_3, "num_bg", 175, 15, TradePlayerBaseAtt.ResPath.."1900015004.png")
        GUI:setContentSize(num_bg, {width = 110, height = 26})
        GUI:Image_setScale9Slice(num_bg, 30, 30, 6, 6)
        GUI:setAnchorPoint(num_bg, 0, 0.5)
    ...
    if not item_4 then
        item_4 = GUI:Layout_Create(List, "item_4", 0, 0, TradePlayerBaseAtt.attachW  - 10, 30, true)
        local num_bg = GUI:Image_Create(item_4, "num_bg", 175, 15, TradePlayerBaseAtt.ResPath.."1900015004.png")
        GUI:setContentSize(num_bg, {width = 110, height = 26})
        GUI:Image_setScale9Slice(num_bg, 30, 30, 6, 6)
        GUI:setAnchorPoint(num_bg, 0, 0.5)
