...
function MergeBag.Init()
    ...
    MergeBag._selPage    = 1     -- 当前选中的页签
    MergeBag._selType    = 1     -- 1: 人物背包; 2: 英雄背包 
...
function MergeBag.main( type,page )
    ...
    GUI:Layout_setBackGroundColor(TouchBg, "#008000")
    GUI:Layout_setBackGroundColorType(TouchBg, 1)
    GUI:Layout_setBackGroundColorOpacity(TouchBg, 255)
    ...
    SL:RegisterLUAEvent(LUA_EVENT_REF_ITEM_LIST, "BagReset", MergeBag.RefreshItemList)
    SL:RegisterLUAEvent(LUA_EVENT_ITEM_POS_CHANGE, "BagPosChange", MergeBag.ItemPosChange)
    SL:RegisterLUAEvent(LUA_EVENT_PLAYER_EQUIP_CHANGE, "EquipChange", MergeBag.RefItemBeStrongMask)
    SL:RegisterLUAEvent(LUA_EVENT_BAG_ITEM_CAHNGE, "BagItemChange", MergeBag.BagItemChange)

    SL:RegisterLUAEvent(LUA_EVENT_HERO_EQUIP_CHANGE, "EquipChange", MergeBag.RefItemBeStrongMask)
    SL:RegisterLUAEvent(LUA_EVENT_REF_HERO_ITEM_LIST, "BagReset", MergeBag.RefreshItemList)
    SL:RegisterLUAEvent(LUA_EVENT_HERO_ITEM_POS_CHANGE, "BagPosChange", MergeBag.ItemPosChange)
    SL:RegisterLUAEvent(LUA_EVENT_HERO_BAG_ITEM_CAHNGE, "BagItemChange", MergeBag.BagItemChange)

    SL:RegisterLUAEvent(LUA_EVENT_ITEM_MOVING, "Moving", MergeBag.OnItemMoving)
    SL:RegisterLUAEvent(LUA_EVENT_ITEM_MOVE_END, "MoveEnd", MergeBag.OnItemMoveEnd)
end
...
function MergeBag.InitPage(parent)
    SL:release_print("背包最大格子数量：", MergeBag._openNum)
...
function MergeBag.SetButtonStatus()
    for i=1,MergeBag._bagPage do
        ...
        if btnPage then
            ...
            GUI:Button_setBrightEx(btnPage, not isPress)
...
function MergeBag.LoadUnOpenGrid(lockNum)
    ...
    for i=clockPos, MergeBag._PerPageNum do
        ...
        local lock = GUI:Clone(MergeBag._UI_ImageLock)
        GUI:setPosition(lock, posX, posY)
        GUI:setVisible(lock, true)
        GUI:addChild(MergeBag._UI_ScrollView, lock)
...
function MergeBag.RefreshItemList()
    ...
    if not MergeBag.IsHeroType() then
        -- startPos: 开始的位置
...
function MergeBag.CreateItem(data, pos)
    local YPos = math.floor((pos-1) / MergeBag._Col)
    ...
    local ItemShow = GUI:ItemShow_Create(MergeBag._UI_ScrollView, tostring(data.MakeIndex), posX, posY, {
        ...
        movable = false, 
        noMouseTips = true,
        from = MergeBag.IsHeroType() and SL:GetItemForm().HERO_BAG or SL:GetItemForm().BAG,
        checkPower = true,
        starLv = true
    })

    GUI:ItemShow_addReplaceClickEvent(ItemShow, function ()
        release_print("ItemShow_addReplaceClickEvent...")
        -- 人物和英雄背包互取
    ...
    GUI:ItemShow_addDoubleEvent(ItemShow, function ()
        release_print("ItemShow_addDoubleEvent...")
        SL:CloseItemTips()
        local status = SL:GetNpcStorageStatus()
        if status > 0 then
            SL:release_print("-----双击--正常存取--------")
            SL:SaveItemToNpcStorage(data)
        else
            -- 使用道具
...
function MergeBag.BagItemChange(data)
    ...
    elseif optType == 3 then
        for k,v in pairs(itemData) do
            ...
            if item then
                GUI:ItemShow_updateItemShow(item, v.item)
