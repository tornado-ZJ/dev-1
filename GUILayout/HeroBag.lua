...
function HeroBag.main( )
    ...
    SL:RegisterLUAEvent(LUA_EVENT_HERO_EQUIP_CHANGE, "EquipChange", HeroBag.RefItemBeStrongMask)
    SL:RegisterLUAEvent(LUA_EVENT_REF_HERO_ITEM_LIST, "BagReset", HeroBag.RefreshItemList)
    SL:RegisterLUAEvent(LUA_EVENT_HERO_ITEM_POS_CHANGE, "BagPosChange", HeroBag.ItemPosChange)
    SL:RegisterLUAEvent(LUA_EVENT_HERO_BAG_ITEM_CAHNGE, "BagItemChange", HeroBag.BagItemChange)
end
    ...
    GUI:ItemShow_addPressEvent(ItemShow, function ()
        ...
        if itemOverLapCount then
            ...
            local function callback(btnType, editparam)
                local editStr = editparam.editStr or ""
                if string.len(editStr) > 0 then
                    local num = tonumber(editparam.editStr) or 0
                    if num > data.OverLap or num < 1 then
                        return SL:ShowSystemTips("请输入正确数量!")
                    end
                    SL:SplitHeroItem(data.MakeIndex, num)
                end
            end
...
function HeroBag.BagItemChange(data)
    ...
    elseif optType == 3 then
        ...
        local startPos = HeroBag._startPos - 1
        local endPos   = HeroBag._endPos - 1
        for k,v in pairs(itemData) do
