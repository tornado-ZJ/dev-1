...
function PlayerHeroBaseAtt.main()
    local parent  = GUI:Attach_Parent()
    local attachW = 348
    local attachH = 478
    PlayerHeroBaseAtt.attachW = attachW

    local _ResPath = "res/private/player_main_layer_ui/player_main_layer_ui_mobile/"
    PlayerHeroBaseAtt.ResPath = _ResPath
    ...
    PlayerHeroBaseAtt.ListView = ListView

    PlayerHeroBaseAtt.RefreshAttr(true)
    
    SL:RegisterLUAEvent("LUA_EVENT_REFBASEATTR", "HeroAttr", PlayerHeroBaseAtt.RefreshAttr)
end

function PlayerHeroBaseAtt.RefreshAttr(isJump)
    local data = SL:GetHeroData()
    local List = PlayerHeroBaseAtt.ListView

    -- 职业
    local jobName = data.jobName
    local item_1 = GUI:getChildByName(List, "item_1")
    if not item_1 then
        item_1 = GUI:Layout_Create(List, "item_1", 0, 0, PlayerHeroBaseAtt.attachW  - 10, 30, true)
        local num_bg = GUI:Image_Create(item_1, "num_bg", 175, 15, PlayerHeroBaseAtt.ResPath.."1900015004.png")
    ...
    if not item_2 then
        item_2 = GUI:Layout_Create(List, "item_2", 0, 0, PlayerHeroBaseAtt.attachW  - 10, 30, true)
        local num_bg = GUI:Image_Create(item_2, "num_bg", 175, 15, PlayerHeroBaseAtt.ResPath.."1900015004.png")
        GUI:setContentSize(num_bg, {width = 110, height = 26})
        GUI:Image_setScale9Slice(num_bg, 30, 30, 6, 6)
        GUI:setAnchorPoint(num_bg, 0, 0.5)
    ...
    if not item_3 then
        item_3 = GUI:Layout_Create(List, "item_3", 0, 0, PlayerHeroBaseAtt.attachW  - 10, 30, true)
        local num_bg = GUI:Image_Create(item_3, "num_bg", 175, 15, PlayerHeroBaseAtt.ResPath.."1900015004.png")
        GUI:setContentSize(num_bg, {width = 110, height = 26})
        GUI:Image_setScale9Slice(num_bg, 30, 30, 6, 6)
        GUI:setAnchorPoint(num_bg, 0, 0.5)
    ...
    if not item_4 then
        item_4 = GUI:Layout_Create(List, "item_4", 0, 0, PlayerHeroBaseAtt.attachW  - 10, 30, true)
        local num_bg = GUI:Image_Create(item_4, "num_bg", 175, 15, PlayerHeroBaseAtt.ResPath.."1900015004.png")
        GUI:setContentSize(num_bg, {width = 110, height = 26})
        GUI:Image_setScale9Slice(num_bg, 30, 30, 6, 6)
        GUI:setAnchorPoint(num_bg, 0, 0.5)
